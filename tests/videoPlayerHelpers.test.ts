...
test('scrub seeks are committed during direct playback only after a short interval', () => {
  assert.equal(shouldCommitScrubSeek({ streamIsTranscoded: false, nowMs: 140, lastCommitMs: 0 }), true);
  assert.equal(shouldCommitScrubSeek({ streamIsTranscoded: false, nowMs: 80, lastCommitMs: 0 }), false);
  assert.equal(shouldCommitScrubSeek({ streamIsTranscoded: true, nowMs: 200, lastCommitMs: 0 }), false);
});
