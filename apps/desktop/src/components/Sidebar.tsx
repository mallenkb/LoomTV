...
export function AnimeIcon({ className, solid = false }: { className?: string; solid?: boolean }) {
  return (
    <svg
...
export function AnimeSolidIcon({ className }: { className?: string }) {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
...
export function FilmNavIcon({ className }: { className?: string }) {
...
export function TVNavIcon({ className }: { className?: string }) {
...
export function TVNavSolidIcon({ className }: { className?: string }) {
...
export function FilmNavSolidExactIcon({ className }: { className?: string }) {
...
function BookmarkSolidIcon({ className }: { className?: string }) {
  return <LucideBookmark className={className} fill="currentColor" aria-hidden="true" />;
}
...
const archiveIcons = { regular: ArchiveNavIcon, fill: ArchiveNavSolidIcon };
const animeIcons = { regular: AnimeIcon, fill: AnimeSolidIcon };
const bookmarkIcons = { regular: LucideBookmark, fill: BookmarkSolidIcon };
const compassIcons = { regular: DiscoverIcon, fill: DiscoverActiveIcon };
const filmIcons = { regular: FilmNavIcon, fill: FilmNavSolidExactIcon };
const folderIcons = { regular: FolderNavIcon, fill: FolderNavSolidIcon };
const homeIcons = { regular: HomeSmileIcon, fill: HomeSmileSolidIcon };
const settingsIcons = { regular: SettingsNavExactIcon, fill: SettingsNavSolidExactIcon };
const televisionIcons = { regular: TVNavIcon, fill: TVNavSolidIcon };

const librarySidebarNavItemIds: LibrarySidebarNavItemId[] = ['anime', 'tv', 'movies'];
const modernVideoCategoryItems: readonly ModernCategoryItem[] = [
  ...
  { label: 'My List', path: '/my-list', routePrefix: '/my-list', folderKey: null, icon: bookmarkIcons.regular, activeIcon: bookmarkIcons.fill, iconOnly: true },
];

const homeNavItem: SidebarNavItem = { id: 'home', path: '/', label: 'Home', icon: homeIcons.regular, activeIcon: homeIcons.fill };
const myListNavItem: SidebarNavItem = { id: 'my-list', path: '/my-list', label: 'My List', icon: bookmarkIcons.regular, activeIcon: bookmarkIcons.fill };
const discoverNavItem: SidebarNavItem = { id: 'discover', path: '/discover', label: 'Discover', icon: compassIcons.regular, activeIcon: compassIcons.fill };
const dividerNavItem: SidebarNavItem = { id: 'divider', path: '', label: 'Library divider', icon: EmptyNavIcon, kind: 'divider' };
const settingsNavItem: SidebarNavItem = { id: 'settings', path: '/settings', label: 'Settings', icon: settingsIcons.regular, activeIcon: settingsIcons.fill };

const sidebarNavItems: Record<LibrarySidebarNavItemId, SidebarNavItem> = {
  anime: { id: 'anime', path: '/anime', label: 'Anime', icon: animeIcons.regular, activeIcon: animeIcons.fill },
  tv: { id: 'tv', path: '/tv', label: 'TV Shows', icon: televisionIcons.regular, activeIcon: televisionIcons.fill },
  movies: { id: 'movies', path: '/movies', label: 'Movies', icon: filmIcons.regular, activeIcon: filmIcons.fill },
...
function ModernCategoryPill({ pathname }: { pathname: string }) {
  ...
  const visibleCategories = modernVideoCategoryItems.filter((category) => {
  ...
  return (
    <header className="loom-modern-header loom-no-drag fixed inset-x-0 top-6 z-50 flex h-12 items-center justify-center px-5">
      <Link to="/" className="loom-modern-mobile-brand" aria-label="Loom home">
        <LoomLogo className="h-8 w-auto" />
      </Link>
      <nav
        className="loom-modern-category-pill loom-no-drag flex h-12 items-center rounded-full border p-1 backdrop-blur-xl"
    ...
    <div ref={menuRef} className={cn('relative', compact ? 'w-12' : 'min-w-0 flex-1')}>
      ...
      {menuOpen && (
        ...
        >
            ...
            {selectableProfiles.map((profile) => {
              ...
              return (
                ...
                >
                  ...
                  {profile.hasPin && <LockKeyhole className="h-3.5 w-3.5 shrink-0 text-[var(--loom-muted)]" aria-hidden="true" />}
                  {isActive && <Check strokeWidth={3} className="h-4 w-4 shrink-0 text-[var(--loom-accent)]" aria-hidden="true" />}
                </button>
              );
            })}
          </div>
          ...
          >
            <span className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[var(--loom-surface-2)]">
              <RefreshCw className={cn('h-4 w-4', isScanning && 'animate-spin')} aria-hidden="true" />
            </span>
...
export default function Sidebar() {
  ...
  const sidebarHighlightActiveId = activeSidebarItemId;
    ...
    return (
      <>
        ...
        <aside className="loom-modern-sidebar loom-no-drag fixed inset-y-0 left-0 z-50 flex w-20 flex-col items-center py-5">
          <nav className="loom-modern-sidebar-desktop-nav mt-6 flex min-h-0 w-full flex-1 flex-col items-center" aria-label="Primary navigation">
            <SharedListHighlight
              activeId={sidebarHighlightActiveId}
              followPointer={false}
            ...
            >
              ...
              {(() => {
                ...
                if (item.kind === 'divider') {
                  return <div key={`modern-${item.id}`} className="my-1 h-px w-8 bg-[var(--loom-text)] opacity-[0.22]" aria-hidden="true" />;
                }
  ...
  return (
    <aside className="h-full w-48 bg-[var(--loom-sidebar)] flex flex-col">
      <div className="loom-sidebar-brand relative bg-transparent p-4">
        ...
        <Link to="/" className="loom-no-drag relative z-10 inline-flex h-10 items-center transition-opacity hover:opacity-85" aria-label="Loom home">
          <LoomLogo className="h-8 w-auto" />
        </Link>
      </div>
      <nav className="min-h-0 flex-1 p-3 pt-0 flex flex-col">
        {libraryActionError ? <span role="alert" className="sr-only">{libraryActionError}</span> : null}
        <SharedListHighlight activeId={sidebarHighlightActiveId} className="loom-shared-highlight-sidebar relative min-h-0 flex-1 overflow-y-auto">
          {(() => {
            return [homeNavItem, ...orderedSidebarItems].map((item) => {
            if (item.kind === 'divider') {
              return <div key={`sidebar-${item.id}`} className="my-2 h-px bg-[var(--loom-panel-border)]" aria-hidden="true" />;
            }
            const isActive = isSidebarItemActive(item.id);
            const Icon = isActive ? (item.activeIcon || item.icon) : item.icon;
            const usesPhosphorNavIcon = item.id.startsWith('live:') || item.id.startsWith('stremio:');
