...
export default function WatchedToggle({ watched, onToggle, className = '', label, surface = 'default' }: WatchedToggleProps) {
  ...
  return (
    <button
      ...
      className={`grid h-9 w-9 shrink-0 place-items-center rounded-full text-white/75 transition-colors ${surfaceClassName} ${defaultHoverClassName} hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[var(--loom-accent)] ${className}`}
    >
      {watched ? (
