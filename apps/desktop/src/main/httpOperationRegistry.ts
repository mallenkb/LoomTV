...
export const httpOperations = {
  ...
  rendererArtworkSave: operation({ method: 'POST', path: '/api/renderer/artwork', owner: 'artwork', scope: 'desktop', requestSchema: z.object({ mediaId: nonEmptyString.max(512), target: z.enum(['thumbnail', 'cover']), dataUrl: z.string().max(25 * 1024 * 1024) }) }),
  ...
  rendererArtworkApply: operation({ method: 'POST', path: '/api/renderer/artwork/apply-official', owner: 'artwork', scope: 'desktop', requestSchema: z.object({ mediaId: nonEmptyString.max(512), candidate: z.object({ id: nonEmptyString.max(512) }), target: z.enum(['all', 'poster', 'cover', 'episodes']).optional() }) }),
