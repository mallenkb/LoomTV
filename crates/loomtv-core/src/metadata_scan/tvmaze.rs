...
fn search_title(title: &str) -> String {
    ...
    let title = SEASON.get_or_init(|| regex::Regex::new(r"(?i)\s+(?:s\d{1,2}|season\s+\d{1,2})\s*$").unwrap())
        .replace(title.trim(), "").trim().trim_end_matches(['(', '[']).trim().to_owned();
    ...
    if let Some(number) = words.first().and_then(|word| word.parse::<u32>().ok()) {
        ...
        if valid && spelled == number { return number.to_string(); }
    }
    title
}

async fn request(gateway: &MetadataProviderGateway, settings: &Value, path: String, query: Value) -> Result<Value> {
    gateway.request_metadata_provider(&json!({"provider":"tvmaze","path":path,"query":query}), settings).await
}

pub(super) async fn fetch(gateway: &MetadataProviderGateway, settings: &Value, item: &ItemSnapshot, cancelled: &AtomicBool) -> Result<Option<MetadataPatch>> {
    check_cancelled(cancelled)?;
    let id = item.provider_ids.get("tvmazeId").and_then(value_id).filter(|id| valid_numeric_id(id));
    let show = if let Some(id) = id {
        ...
        let matches: Vec<_> = results.as_array().into_iter().flatten().map(|hit| &hit["show"]).filter(|show| {
            let name = text(show.get("name"));
            (normalize_title(&name) == normalize_title(&title)
                || name.split_once(':').is_some_and(|(base, _)| normalize_title(base) == normalize_title(&title)))
                && (item.year <= 0 || text(show.get("premiered")).get(..4).and_then(|year| year.parse::<i64>().ok()).is_none_or(|year| year == item.year))
        }).collect();
        if matches.len() != 1 { return Ok(None); }
        matches[0].clone()
    };
    let Some(id) = value_id(&show["id"]) else { return Ok(None); };
    let mut patch = MetadataPatch {
        title: nonempty(text(show.get("name"))),
        summary: nonempty(strip_markup(&text(show.get("summary")))),
        year: text(show.get("premiered")).get(..4).and_then(|year| year.parse().ok()),
        rating: positive_number(show["rating"].get("average")),
        poster: nonempty(text(show["image"].get("original"))),
        genres: show["genres"].as_array().into_iter().flatten().filter_map(Value::as_str).map(str::to_owned).collect(),
        ..Default::default()
    };
    ...
    if item.requested.contains(&Category::Episodes) {
        if let Ok(episodes) = request(gateway, settings, format!("shows/{id}/episodes"), json!({"specials":1})).await {
            for episode in episodes.as_array().into_iter().flatten().take(MAX_LOCAL_EPISODES) {
                let (Some(season), Some(number)) = (integer(episode.get("season")), integer(episode.get("number"))) else { continue; };
    ...
    if item.requested.contains(&Category::Cast) {
        if let Ok(cast) = request(gateway, settings, format!("shows/{id}/cast"), json!({})).await {
            patch.cast = cast.as_array().into_iter().flatten().take(MAX_CAST).map(|entry| json!({
    ...
    if item.requested.contains(&Category::Artwork) {
        if let Ok(images) = request(gateway, settings, format!("shows/{id}/images"), json!({})).await {
