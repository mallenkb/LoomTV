...
An interrupted release remains a draft. A rerun may replace that draft only after the prior release JSON/assets have been archived both as a 90-day workflow artifact and as a permanent, uniquely named draft evidence release; it refuses to replace an already-published release or move its protected tag. Evidence drafts are audit records: never publish or delete them. The uploaded `SHA256SUMS` and `release-manifest.json` are checked before the draft is made public, while cryptographic provenance is supplied by GitHub artifact attestations with the SLSA provenance predicate rather than a repository-authored provenance file. Each manifest tuple names one of three platform-specific reusable attestation workflows, and publication verifies that exact signer workflow and workflow digest.
